...
public final class ExecuteCommandHandler extends CommandHandler {
    @Override
    protected String getCommandHeader() {
        return ExecuteCommand.COMMAND_HEADER;
    }
    ...
    public ParsingResult build(List<TokenType> tokens, ParserContext parserContext) {
        ExecuteCommandBuilder ecb = ExecuteCommand.builder();
        for (TokenType tt : tokens) {
            switch(tt) {
                case TokenType.Value(int cycle) -> {
                    if (cycle <= 0) {
                        return ParsingResult.illegalValue(this.getCommandHeader(), cycle, "cycle > 0");
                    }
                    ecb.setCycle(cycle);
                }
                case TokenType.Command ignored -> {}
