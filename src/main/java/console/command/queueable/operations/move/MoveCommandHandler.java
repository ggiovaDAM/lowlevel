...
public final class MoveCommandHandler extends CommandHandler {
    @Override
    protected String getCommandHeader() {
        return MoveCommand.COMMAND_HEADER;
    }
    ...
    protected ParsingResult build(List<TokenType> tokens, ParserContext parserContext) {
        MoveCommandBuilder scb = MoveCommand.builder();
        int registryStage = 0;
        for (TokenType tt : tokens) {
            switch (tt) {
