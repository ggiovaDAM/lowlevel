...
public final class CopyCommandHandler extends CommandHandler {
    
    @Override
    protected String getCommandHeader() {
        return CopyCommand.COMMAND_HEADER;
    }
    ...
    protected ParsingResult build(List<TokenType> tokens, ParserContext parserContext) {
        CopyCommandBuilder scb = CopyCommand.builder();
        int registryStage = 0;
        for (TokenType tt : tokens) {
            switch (tt) {
                ...
                case TokenType.RegistryKey(String key) -> {
                    if (registryStage == 0) { scb.setRegistry(key); }
                    else                    { scb.addKey(key); }
                    registryStage++;
                }
                case TokenType.Value(int value) -> {
                    ...
                    scb.setCycle(value, parserContext.cycle().getCycle());
                }
                default -> {}
            }
        }
    ...
    protected List<TokenOrder> getTokenOrder() {
        return List.of(
                ...
                new TokenOrder.OneOrMore(new TokenType.RegistryKey(null)),
